...
public class Utils
{
    ...
    public static int StrDateDiffSeconds(string time, int Sec)
    {
        if (Utils.StrIsNullOrEmpty(time))
            return 1;
        DateTime dateTime = Cast.StrToDateTime(time, DateTime.Parse("1900-01-01"));
    ...
    {
        return Cast.StrToBool(expression, defValue);
    }
    ...
    {
        return Cast.StrToFloat(strValue, defValue);
    }
    ...
    /// <summary>
    /// 通过文本中设置ListControl被选中，没有对应项项自动添加到条记录上
    /// </summary>
    /// <param name="control"></param>
    /// <param name="text">text</param>
    /// <param name="defaultValue">List中不存在，text对应的值</param>
    public static void SetListControlByText(ListControl control, string text, string defaultValue = "")
    {
        bool isSel = false;
        foreach (ListItem it in control.Items)
        {
            if (it.Text == text)
            {
                it.Selected = true;
        ...
        if (isSel == false)
        {
            control.Items.Insert(0, new ListItem(text, defaultValue));
        }
    }
    ...
    public static void FillEmptyToList<T>(ref List<T> lst, int totalRow)
    {
        if (lst == null)
        {
            lst = new List<T>();
        }
        if (totalRow > lst.Count)
        {
            int fillRow = totalRow - lst.Count;
            for (int i = 0; i < fillRow; i++)
            {
                lst.Add(Activator.CreateInstance<T>());
            }
        }
